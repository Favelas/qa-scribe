...
4. **PATCH** the relevant skill (`SKILL.md` and/or `reference.md`) with that rule. Bump patch or minor version in the skill header (semver). Start family was **v1.0.0**. Patch the copy under `.claude/skills/<generator>/` (the one Claude Code actually runs). If `.cursor/skills/<generator>/` still exists for Cursor parity, apply the same patch there too so the two stay in sync.
