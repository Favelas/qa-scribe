...
- Pack-wide, not just when isolation/RBAC is in scope: the pack **must** contain at least one happy-path (EP) case, one negative-path (NEG) case, and one boundary/edge (BVA) case, or state `Not applicable: <reason>` if the feature truly has no invalid-input surface. Do not rely on isolation/RBAC scenarios alone to satisfy this.
- Cross-check against the risk register: every Critical and High risk in scope must have at least one case tracing to it. A risk register with an uncovered Critical/High is a gap in the pack, not just in the register — flag it rather than silently generating cases for easier risks only.
...
`standards/rubrics/cases.md`. Immediate fail: missing REQ or RSK; no isolation search case when two companies are in scope; no role-button case when RBAC is in scope; >8 steps; mixed features; pack has no happy (EP), negative (NEG), and boundary (BVA) case between them with no `Not applicable` reason; any Critical/High risk from the register left with zero traced cases.
